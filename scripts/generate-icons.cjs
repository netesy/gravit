...
const dir = path.join(__dirname, '..', 'public', 'assets', 'img', 'new-document');
...
for (const [name, elements] of Object.entries(icons)) {
  fs.writeFileSync(path.join(dir, `preset-${name}-white.svg`), makeSvg(elements, 'rgb(173,173,173)'));
  fs.writeFileSync(path.join(dir, `preset-${name}-black.svg`), makeSvg(elements, 'rgb(114,114,114)'));
